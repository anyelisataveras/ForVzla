-- Admin con Supabase Auth (magic link) — reemplaza moderación por PIN

-- Quitar RPCs con PIN
drop function if exists aprobar_post_redes(uuid, text);
...
begin
  ...
  return exists (
    select 1 from admin_users au where lower(au.email) = lower(v_email)
  );
end;
$$;
...
-- Centros y edificios: solo admins escriben (lectura pública)
...
-- Posts: solo admins actualizan (scraper inserta con service_role)
do $$ begin
  if exists (
...
-- ── Aprobar post (auth admin, sin PIN) ───────────────────────
...
begin
  ...
  if p_destino not in ('necesidad', 'edificio', 'ambos') then
  ...
  if p_destino in ('edificio', 'ambos') then
    if p.lat is null or p.lng is null then
      raise exception 'Faltan coordenadas para crear edificio';
    end if;
    insert into edificios_colapsados (
      nombre, zona, lat, lng, estado_edificio, personas_atrapadas, fuente, notas
    ) values (
  ...
  if p_destino in ('necesidad', 'ambos') then
    if p.lat is not null and p.lng is not null then
      ...
      if v_dup_id is not null then
        ...
        return jsonb_build_object(
          'ok', true, 'necesidad_id', v_dup_id, 'edificio_id', v_edif_id,
          'accion', 'confirmado_existente'
        );
      end if;
    end if;
    ...
    insert into necesidades (
      ...
      nombre_contacto, telefono, fuente, source_url, source_hash,
      validada, estado, edificio_id
    ) values (
      ...
      true, 'pendiente', v_edif_id
    ) returning id into v_nec_id;
  end if;

  update posts_redes
    set estado = 'aprobado',
        necesidad_id = coalesce(v_nec_id, necesidad_id),
        revisado_at = now()
    where id = p_post_id;

  return jsonb_build_object(
    'ok', true,
    'necesidad_id', v_nec_id,
    'edificio_id', v_edif_id,
    'accion', 'insertado'
  );
end;
$$;
