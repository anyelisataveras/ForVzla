...
-- ── VOLUNTARIOS POR GRUPO (Cuidadoras Caracas, etc.) ──
...
create policy "admin_read_voluntarios"
  on voluntarios for select using (is_admin());

create policy "admin_update_voluntarios"
  on voluntarios for update using (is_admin());

create policy "admin_delete_voluntarios"
  on voluntarios for delete using (is_admin());

create or replace function voluntario_existe(p_grupo text, p_id_dni text)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from voluntarios where grupo = p_grupo and id_dni = trim(p_id_dni));
$$;
...
begin
  ...
  for r in
    select id from (
      select id, row_number() over (partition by grupo, numero_voluntaria order by created_at, id) as rn
      from voluntarios
...
-- ── BRIGADAS CUIDADORAS (infograma Dari) ──
...
create policy "pub_read_brigadas" on brigadas for select using (activa = true);
drop policy if exists "admin_all_brigadas" on brigadas;
create policy "admin_all_brigadas" on brigadas for all using (is_admin()) with check (is_admin());
...
returns text[] language sql stable security definer set search_path = public as $$
  ...
  from brigadas b where b.grupo = p_grupo and b.activa = true
    and b.slug in (select trim(x) from jsonb_array_elements_text(coalesce(p_brigadas, '[]'::jsonb)) as t(x) where trim(x) <> '');
$$;
grant execute on function normalizar_brigadas_voluntario(text, jsonb) to anon, authenticated;
insert into brigadas (grupo, slug, nombre, mision, acciones, requisitos, icono, color_bg, color_fg, orden) values
  ('cuidadoras_caracas','logistica_alimentos','Brigada de Logística de Alimentos','Preparar, trasladar y distribuir comidas de forma organizada.','Preparación, traslado y distribución organizada de desayunos, almuerzos y cenas.','Experiencia en cocina o logística de alimentos deseable. No es obligatorio.','🍲','#D6EAF8','#1A5276',1),
  ('cuidadoras_caracas','salud_medicamentos','Brigada de Salud y Medicamentos','Atención médica segura.','Control del inventario médico y entrega supervisada de tratamientos a pacientes crónicos (hipertensión, diabetes).','Profesión de salud o experiencia en primeros auxilios deseable.','💊','#D5F5E3','#186A3B',2),
  ('cuidadoras_caracas','clasificacion_donaciones','Brigada de Clasificación de Donaciones','Gestión de insumos donados.','Recepción, conteo y organización (por tallas y necesidades) de ropa, artículos del hogar y kits de limpieza.','Ninguno — todas pueden colaborar.','📦','#FCF3CF','#7D6608',3),
  ('cuidadoras_caracas','saneamiento','Brigada de Saneamiento y Mantenimiento','Espacios habitables.','Limpieza y orden de áreas comunes para garantizar un ambiente higiénico y digno.','Ninguno — todas pueden colaborar.','🧹','#FAE5D3','#A04000',4),
  ('cuidadoras_caracas','recreacion','Brigada de Atención Directa y Recreación','Ocio y movimiento para la comunidad.','Organización de actividades deportivas, culturales y recreativas para mantener activa a la comunidad.','Experiencia con niños deseable. No es obligatorio.','⚽','#FADBD8','#922B21',5),
  ('cuidadoras_caracas','contencion','Brigada de Contención Emocional y Resguardo','Apoyo psicoafectivo.','Acompañamiento, escucha y protección brindada por madres voluntarias, con especial enfoque en los niños.','Formación en psicología, trabajo social o experiencia en contención deseable.','💜','#E8DAEF','#6C3483',6),
  ('cuidadoras_caracas','social','Brigada Social','Atención integral a familias.','Identificación de casos vulnerables, enlace con otras brigadas y atención general a las familias.','Ninguno — todas pueden colaborar.','🤝','#D1F2EB','#117A65',7)
on conflict (grupo, slug) do update set nombre=excluded.nombre,mision=excluded.mision,acciones=excluded.acciones,requisitos=excluded.requisitos,icono=excluded.icono,color_bg=excluded.color_bg,color_fg=excluded.color_fg,orden=excluded.orden,activa=true;
...
-- ── Sitios + jornadas (actividades) — ver migrations/20250706160000_sitios_jornadas_seed.sql ──
