...
> **Costo:** $0/mes. **Despliegue:** Vercel (carpeta `public/`).
...
Usuarios móviles
      │
 public/index.html (PWA, 1 archivo)
...
│  ├─ manifest.json
│  └─ icon.svg
├─ package.json            # dev local + Vercel
├─ vercel.json             # deploy estático → public/
├─ .cursorrules
├─ .env.example
├─ HANDOFF.md
├─ supabase/
│  ├─ config.toml
│  └─ migrations/          # migraciones versionadas (supabase db push)
├─ db/
│  └─ setup_supabase.sql   # script monolítico para SQL Editor manual
└─ scraper/
   ├─ ingesta_redes.js
   ├─ package.json
   └─ README_INGESTA.md
```
> La app vive en `public/`. Las migraciones van en `supabase/migrations/`. `db/setup_supabase.sql` es respaldo para ejecutar todo de una vez en el SQL Editor.
